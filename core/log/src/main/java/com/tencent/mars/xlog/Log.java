...
public class Log {
    ...
    private static final LogImp debugLog = new LogImp() {
        ...
        @Override
        public void write(long logInstancePtr, String message) {
        ...
        @Override
        public long openLogInstance(int level, int mode, String cacheDir, String logDir, String nameprefix, int cacheDays, String pubKey) {
            return 0;
        }
    ...
    static {
        ...
        try {
            sb.append("Android SDK_INT:[").append(Build.VERSION.SDK_INT);
            ...
            sb.append("] ABIS:").append(Arrays.toString(Build.SUPPORTED_ABIS));
        } catch (Throwable e) {
    ...
    public static void write(final String format) {
        if (logImp != null && logImp.getLogLevel(0) <= LEVEL_ERROR) {
            String log = format;
            if (log == null) {
                log = "";
    ...
    public static LogInstance openLogInstance(int level, int mode, String cacheDir, String logDir, String nameprefix, int cacheDays, String pubKey) {
        synchronized (sLogInstanceMap) {
            if (sLogInstanceMap.containsKey(nameprefix)) {
                return sLogInstanceMap.get(nameprefix);
            }
            LogInstance instance = new LogInstance(level, mode, cacheDir, logDir, nameprefix, cacheDays, pubKey);
    ...
    public interface LogImp {
        ...
        void write(long logInstancePtr, String message);

        int getLogLevel(long logInstancePtr);

        void setAppenderMode(long logInstancePtr, int mode);

        long openLogInstance(int level, int mode, String cacheDir, String logDir, String nameprefix, int cacheDays, String pubKey);
    ...
    public static class LogInstance {
        ...
        private LogInstance(int level, int mode, String cacheDir, String logDir, String nameprefix, int cacheDays, String pubKey) {
            if (logImp != null) {
                mLogInstancePtr = logImp.openLogInstance(level, mode, cacheDir, logDir, nameprefix, cacheDays, pubKey);
            }
        }
        ...
        public void write(String message) {
            if (logImp != null && mLogInstancePtr != -1) {
                logImp.write(mLogInstancePtr, message);
            }
        }
